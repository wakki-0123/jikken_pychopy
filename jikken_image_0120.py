...
def psychopy00():
    ...
    imageLists = glob.glob(cwd + "/jikken_0120_1/*.jpg") # 任意のファイルがあるディレクトリ
    return imageLists

def psychopy11():
    cwd = os.getcwd()  # 現在の作業ディレクトリ
    imageLists = glob.glob(cwd + "/jikken_0120_2/*.jpg") # 任意のファイルがあるディレクトリ
    return imageLists
...
if __name__ == "__main__":
    ...
    imageData1 = psychopy1(imageLists1)
    ...
    while True:
        ...
        time3 = time2 - time1
        if int(time3) == 30: # クリックしてから30秒後に画像提示をするようになっている
            psychopy(imageLists, imageData, time3)

        if int(time3) == 240: # クリックしてから240秒後に画像提示をするようになっている
            psychopy(imageLists1, imageData1, time3)
